...
use crate::feeder::{OracleFeeder, SharedState};
...
async fn test_collection_failure_handling() {
    ...
    *ctx.mock_feeder.should_fail_collection.write().await = true;
    ...
    assert!(
        data.is_none(),
        "No data should be present after failed collection"
    );
}
...
async fn test_concurrent_data_access() {
    ...
    assert!(
        data.is_some(),
        "Data should be present after concurrent operations"
    );
}
...
async fn test_miner_health_check_timeout() {
    ...
    assert!(
        !status.available,
        "Timeout should result in unavailable status"
    );
}
...
async fn test_miner_health_check_with_mock_server() {
    ...
    assert!(
        ...
        "Mock uses IP check, not actual server"
    );
...
async fn test_retry_mechanism_on_transient_failures() {
    ...
    *ctx.mock_feeder.should_fail_collection.write().await = false;
    ...
    assert!(
        data.is_some(),
        "Data should be present after successful retry"
    );
}
...
async fn test_operation_timeout_handling() {
    ...
    // Test with timeout
    let result = timeout(Duration::from_secs(5), ctx.mock_feeder.collect_miner_data()).await;
