...
//! Advanced integration tests covering complex real-world scenarios
...
async fn test_partial_miner_recovery() {
    ...
    *shared_state.current_miners_data.lock().await = Some(recovered_miners);
    ...
    let data = shared_state.current_miners_data.lock().await;
    let online_count = data.as_ref().unwrap().iter().filter(|(_, value)| {
        if let OracleValue::MinerStatus(status) = value {
            status.online
        } else {
            false
...
#[tokio::test]
async fn test_transaction_queue_failure_isolation() {
    init_transaction_queue();
    let queue = TRANSACTION_QUEUE.get().unwrap();
    
    // Enqueue failing transaction
    let rx1 = queue.enqueue(|| async {
        Err(Error::custom("Intentional failure").into())
    }).await.unwrap();
    
    // Enqueue succeeding transaction
    let rx2 = queue.enqueue(|| async {
        Ok(TxOutput::OracleFeedSuccess)
    }).await.unwrap();
    ...
    // Allow time for retries to exhaust
    tokio::time::sleep(Duration::from_secs(2)).await;
    
    assert!(result1.is_ok() || result1.is_err()); // Either way is fine for test
...
#[tokio::test]
async fn test_timezone_and_timing_edge_cases() {
    let shared_state = Arc::new(SharedState {
        current_miners_data: Mutex::new(None),
    });
    
    // Simulate operations at different times
    let start = tokio::time::Instant::now();
    
    // Immediate operation
    let _data1 = shared_state.current_miners_data.lock().await;
    
    // Delayed operation
    tokio::time::sleep(Duration::from_millis(50)).await;
    let _data2 = shared_state.current_miners_data.lock().await;
