//! Comprehensive test harness with utilities for end-to-end testing
...
async fn test_e2e_continuous_operation_stability() {
    ...
    for cycle in 0..50 {
        let miners = vec![(
            ...
            OracleValue::MinerStatus(ProcessStatus {
                online: cycle % 5 != 0, // Occasional failures
                available: cycle % 3 != 0,
            }),
        )];
    ...
    let summary = metrics.get_summary().await;
    assert!(summary.successful_collections > 40);
    assert!(summary.failed_collections < 15);
}
...
async fn test_parameterized_failure_rates() {
    ...
    for rate in failure_rates {
        ...
        *shared_state.current_miners_data.lock().await = Some(miners);
        ...
        let data = shared_state.current_miners_data.lock().await;
        let online_count = data.as_ref().unwrap().iter().filter(|(_, v)| {
